...
for i in xrange(1, covarianceSteps+1):
    ...
    for j in xrange(1, covarianceSteps+1):
        ...
        for l in xrange(numRuns) :
            with open(environmentTemplate + ".cfg", 'r') as f:
                ...
                for k in xrange(len(data)):
                    if "processError" in data[k]:
                        ...
                        data[k] = "logPath = " + resultsPath + environmentTemplate + "/" + str(numObstacles) + "_obstacles/" + str(i) + "_proc_" + str(j) + "_obs/ \n"
			...
			data[k] = "measureOutputFileSNM = /data/hoe01h/oppt_devel/files/measureSamplesDubinEmptyEnvironment/measureSamplesDubinEmptyEnvironment" + str(i) + "_proc_" + str(j) + "_obsSNM.txt \n"
                    elif "measureOutputFileMONG" in data[k]:
			data[k] = "measureOutputFileMONG = /data/hoe01h/oppt_devel/files/measureSamplesDubinEmptyEnvironment/measureSamplesDubinEmptyEnvironment" + str(i) + "_proc_" + str(j) + "_obsMONG.txt \n"
