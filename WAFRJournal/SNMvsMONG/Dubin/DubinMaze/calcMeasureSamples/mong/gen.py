...
for i in xrange(1, covarianceSteps+1):
	...
	with open(environmentTemplate + ".cfg", 'r') as f:
		...
		if "processError" in data[k]:
		    ...
		    data[k] = "measureOutputFileSNM = /data/hoe01h/oppt_devel/files/measureSamplesDubinMaze/measureSamplesDubinMaze" + str(i) + "_proc_" + str(i) + "_obsSNM.txt \n"
		elif "measureOutputFileMONG" in data[k]:
		    data[k] = "measureOutputFileMONG = /data/hoe01h/oppt_devel/files/measureSamplesDubinMaze/measureSamplesDubinMaze" + str(i) + "_proc_" + str(i) + "_obsMONG.txt \n"
