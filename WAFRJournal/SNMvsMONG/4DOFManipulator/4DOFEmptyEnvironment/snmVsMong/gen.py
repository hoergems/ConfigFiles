    ...
    observationCovariance = minCovariance    
		...
		with open(environmentTemplate + ".cfg", 'r') as f:
			...
			if "processError" in data[k]:
			    ...
			    data[k] = "measureOutputFileSNM = /data/hoe01h/oppt_devel/files/measureSamplesDubin4DOFEnvironment/measureSamples4DOFEmptyEnvironment" + str(i) + "_proc_" + str(j) + "_obsSNM.txt \n"
			elif "measureOutputFileMONG" in data[k]:
			    data[k] = "measureOutputFileMONG = /data/hoe01h/oppt_devel/files/measureSamplesDubin4DOFEnvironment/measureSamples4DOFEmptyEnvironment" + str(i) + "_proc_" + str(j) + "_obsMONG.txt \n"
